...
exports.renderClassName = exports.renderDartClass = void 0;
...
function renderDartClass(schema, api, options) {
    var _a;
    ...
    var keyword = [prefix, 'class'].filter(Boolean).join(' ');
    var onRenderClassName = function (a, b) { };
...
exports.renderDartClass = renderDartClass;
function _renderDartClass(primitiveValueSchema, api, parentKeyName, isRoot, options) {
    if (parentKeyName === void 0) { parentKeyName = ''; }
    if (isRoot === void 0) { isRoot = false; }
    var prefix = options.prefix, suffixName = options.suffixName, onlyBody = options.onlyBody, dropComment = options.dropComment, emptyBodyCode = options.emptyBodyCode, _a = options.paramType, paramType = _a === void 0 ? 'request' : _a;
    ...
    var commentCode = onlyBody ? '' : dropComment !== true ? renderClassComment(api, paramType, isRoot) : '';
}
function renderClassDeepObject(schema, isRoot, memo, parentProp) {
    var e_1, _a;
    var _b;
    if (isRoot === void 0) { isRoot = false; }
    if (memo === void 0) { memo = new Map(); }
    if (parentProp === void 0) { parentProp = ''; }
    if (memo.has(schema)) {
...
function renderClassComment(api, paramType, isRoot) {
    if (isRoot === void 0) { isRoot = false; }
    var summary = paramType === 'request' ? 'Request data' : paramType === 'response' ? 'Response data' : '';
