...
async function getStdOut(command: any, args: any) {
  return (await exec(command, args)).stdout;
}

function getStdOutSync(command: any, args: any) {
...
function parseLocale(string: any) {
  const env: Recordable = {};
...
export async function osLocale(options = defaultOptions) {
  ...
  try {
    const envLocale: string = getEnvLocale() as string;
...
export function osLocaleSync(options = defaultOptions) {
  ...
  let locale;
  try {
    const envLocale = getEnvLocale() as string;
